    ...
    <form onSubmit={handleSubmit} className="max-w-2xl mx-auto">
      ...
      <div className="space-y-4">
        <div>
          ...
          <Input id="name" name="name" value={formData.name} onChange={handleChange} required placeholder="نام کالا را وارد کنید" />
        </div>
        <div>
          <label htmlFor="code" className="block text-sm font-medium text-gray-700 mb-1">کد کالا</label>
          <Input id="code" name="code" value={formData.code} onChange={handleChange} required placeholder="کد کالا را وارد کنید" />
        </div>
        <div>
          <label htmlFor="equipment_code" className="block text-sm font-medium text-gray-700 mb-1">کد تجهیزات</label>
          <Input id="equipment_code" name="equipment_code" value={formData.equipment_code} onChange={handleChange} placeholder="کد تجهیزات را وارد کنید" />
        </div>
        <div>
          <label htmlFor="unit" className="block text-sm font-medium text-gray-700 mb-1">واحد</label>
          <Input id="unit" name="unit" value={formData.unit} onChange={handleChange} required placeholder="واحد کالا را وارد کنید" />
        </div>
        <div>
          <label htmlFor="model" className="block text-sm font-medium text-gray-700 mb-1">مدل</label>
          <Input id="model" name="model" value={formData.model} onChange={handleChange} placeholder="مدل را وارد کنید" />
        </div>
        <div>
          <label htmlFor="serial_number" className="block text-sm font-medium text-gray-700 mb-1">شماره سریال</label>
          <Input id="serial_number" name="serial_number" value={formData.serial_number} onChange={handleChange} placeholder="شماره سریال را وارد کنید" />
        </div>
        <div>
          <label htmlFor="ownership" className="block text-sm font-medium text-gray-700 mb-1">مالکیت</label>
          <select id="ownership" name="ownership" value={formData.ownership} onChange={handleChange} className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
        ...
        <div>
          ...
          <Input id="location" name="location" value={formData.location} onChange={handleChange} placeholder="موقعیت را وارد کنید" />
        </div>
        <div>
          <label htmlFor="category" className="block text-sm font-medium text-gray-700 mb-1">دسته‌بندی</label>
          <Input id="category" name="category" value={formData.category} onChange={handleChange} placeholder="دسته‌بندی را وارد کنید" />
        </div>
        <div>
          <label htmlFor="calibration_certificate" className="block text-sm font-medium text-gray-700 mb-1">گواهی کالیبراسیون</label>
          <Input id="calibration_certificate" name="calibration_certificate" value={formData.calibration_certificate} onChange={handleChange} placeholder="شماره گواهی کالیبراسیون را وارد کنید" />
        </div>
        <div>
          <label htmlFor="calibration_period_years" className="block text-sm font-medium text-gray-700 mb-1">دوره کالیبراسیون (سال)</label>
          <Input id="calibration_period_years" name="calibration_period_years" type="number" value={formData.calibration_period_years} onChange={handleChange} placeholder="دوره کالیبراسیون را وارد کنید" />
        </div>
        <div>
          <label htmlFor="calibration_place" className="block text-sm font-medium text-gray-700 mb-1">محل کالیبراسیون</label>
          <Input id="calibration_place" name="calibration_place" value={formData.calibration_place} onChange={handleChange} placeholder="محل کالیبراسیون را وارد کنید" />
        </div>
        ...
        <div>
          ...
          <textarea id="description" name="description" value={formData.description} onChange={handleChange} rows="4" className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="توضیحات کالا را وارد کنید" />
        </div>
        <div className="flex gap-4">
          <Button type="submit" variant="primary" disabled={loading}>{loading ? 'در حال ذخیره...' : 'ذخیره'}</Button>
